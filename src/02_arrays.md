...
*Ещё* вопрос --- а елси аимпортировать ``java``? Ответ --- импорт * не рекурсивный. Он только классы из соответствующего пакета.  К примеру при ``import java.lang.*;`` у нас не будет классов из ``java.lang.zip``. Импортировать *пакеты* в Java --- нельзя!
